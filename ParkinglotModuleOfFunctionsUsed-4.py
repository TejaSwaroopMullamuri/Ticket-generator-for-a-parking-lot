...
def validity_of_lots_cars(particular_lot, floors_for_cars, columns_for_cars, rows_for_cars):
    ...
    while i < len(particular_lot):
        if particular_lot[i] != ' ':
            if particular_lot[i].strip() != ',':
                pass
            elif particular_lot[i].strip() == ',' and particular_lot[i].strip() != '':
                if particular_lot[(j + 1):i].strip().isdigit():
                    x.append(int(particular_lot[(j + 1):i].strip()))
                    j = i
                else:
                    return False
        i += 1
...
def unavailable_lots_for_cars(unavailablelot, parkinglot_for_cars, floors_for_cars, columns_for_cars, rows_for_cars):
    ...
    pl_lop=parkinglot_for_cars
    if validity_of_lots_cars(unavailablelot, floors_for_cars, columns_for_cars, rows_for_cars)==True:
        j = -1
        unavailablelot += ','
        x = []
        i = 0
        while i < len(unavailablelot) :
            if unavailablelot[i] != ' ':
                if unavailablelot[i].strip() != ',':
                    pass
                elif unavailablelot[i].strip() == ',' and unavailablelot[i].strip()!='':
                    x.append(int(unavailablelot[j + 1:i].strip()))
                    j = i
            i += 1

        pl_lop[x[0] - 1][x[1] - 1][x[2] - 1][0] = 'Filled forever'
        #print(x)
...
def unavailable_lots_for_bikes(unavailablelot, parkinglot_for_bikes, floors_for_bikes, columns_for_bikes, rows_for_bikes):
    ...
    pl_lop_bikes=parkinglot_for_bikes
    if validity_of_lots_bikes(unavailablelot, floors_for_bikes, columns_for_bikes, rows_for_bikes)==True:
        j = -1
        unavailablelot += ','
        x = []
        i = 0
        ...
        pl_lop_bikes[x[0] - 1][x[1] - 1][x[2] - 1][0] = 'Filled forever'
        #print(x)
        print("Successfully marked this lot unavailable")
        return pl_lop_bikes
    else:
